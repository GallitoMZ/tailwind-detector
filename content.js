(function () {
  ...
  const CDN_HREF_RE = /tailwindcss[@\/]v?(\d+(?:\.\d+\.\d+)?)/i;
  ...
  const MAX_STYLESHEETS_TO_FETCH = 5;
  ...
  function detectExactVersionSync() {
    ...
    for (const link of document.querySelectorAll('link[rel="stylesheet"]')) {
      const m = (link.href || '').match(CDN_HREF_RE);
  ...
  // ---------- Detección asíncrona (versión exacta desde CSS crudo) ----------
  ...
  function prioritizeStylesheets() {
    ...
    return links
      .map((link) => {
        ...
        let score = 0;
        try {
          if (new URL(href).origin === currentOrigin) score += 2;
        } catch {}
        if (/tailwind/i.test(href)) score += 3;
        return { href, score };
      })
  ...
  async function detectExactVersionFromLinks() {
    const candidates = prioritizeStylesheets();
